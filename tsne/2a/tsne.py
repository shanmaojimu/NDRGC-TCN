...
# ========================== Load Data ==========================
tsne_file = "tsne_test_Sub03.csv"   # 3197 samples
...
# ========================== Data Cleaning ==========================
# Replace infs with NaN and drop missing values
...
groups = ['Left Hand', 'Right Hand', 'Feet', 'Tongue']

# Assign colors in the specified order
color_list = ["#A5C2E2", "#FFC080", "#8DBF8D", "#DDA0DD"]
...
# Main title
fig.suptitle('Our Method', fontsize=16, fontweight='bold')

# Use GridSpec to create the marginal plot layout
...
for group, color in sci_colors.items():
    ...
    if not group_df.empty:
        sns.kdeplot(
            data=group_df, x='Dim1', fill=True, alpha=0.4,
            color=color, linewidth=0.5, ax=ax_top
        )
...
for group, color in sci_colors.items():
    ...
    if not group_df.empty:
        sns.kdeplot(
            data=group_df, y='Dim2', fill=True, alpha=0.4,
            color=color, linewidth=0.5, ax=ax_right
        )
...
# ========================== Save & Show ==========================
plt.savefig("tsne_ours.png", dpi=300, bbox_inches='tight')
plt.show()